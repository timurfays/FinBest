...
def main():
    ...
    parser.add_argument("--top_nodes", type=int, default=500, help="Количество топовых узлов для анализа")
    ...
    try:
        ...
        # 8. Выбираем топ-N влиятельных узлов
        logger.info(f"Выбираем топ-{args.top_nodes} влиятельных узлов")
        window_spec = Window.partitionBy().orderBy(F.desc("influence_score"))
        top_nodes = result.withColumn("rank", F.row_number().over(window_spec)) \
            .filter(F.col("rank") <= args.top_nodes) \
            .drop("rank")
        
        # 9. Получаем ребра только между топ-узлами
        logger.info("Фильтруем ребра для топ-узлов")
        top_node_ids = top_nodes.select("id").collect()
        top_node_set = {row["id"] for row in top_node_ids}
        
        # Преобразуем в RDD для фильтрации
        filtered_edges_rdd = final_edges.rdd.filter(
            lambda row: row["src"] in top_node_set and row["dst"] in top_node_set
        )
        ...
        for _, row in vertices_df.iterrows():
            G.add_node(
                ...
                degree=int(row['degree'])  # Изменено с connections на degree
            )
        ...
        stats = {
            ...
            "avg_degree": float(degrees.agg({"degree": "avg"}).collect()[0][0]),  # Изменено с connections на degree
            "max_degree": int(degrees.agg({"degree": "max"}).collect()[0][0]),   # Изменено с connections на degree
